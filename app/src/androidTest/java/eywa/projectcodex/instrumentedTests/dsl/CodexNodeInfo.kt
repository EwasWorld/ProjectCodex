...
/**
 * Helper class for [InstrumentedTestActionDsl].
 * Handles which matchers and actions can be added when.
 * Runs actions with [createNode], [waitForDisplay], and [performActions].
 */
internal sealed class CodexNodeInfo {
    object Empty : CodexNodeInfo() {
        override fun plus(other: CodexNodeAction): CodexNodeInfo {
            return Single() + other
        }

        override fun plus(other: CodexNodeMatcher): CodexNodeInfo {
            return Single() + other
        }
    }
    ...
    data class Single(
        ...
        override fun plus(other: CodexNodeAction): CodexNodeInfo {
            return copy(actions = actions + other)
        }

        override fun plus(other: CodexNodeMatcher): CodexNodeInfo {
            return copy(matchers = matchers + other)
        }
        ...
        override fun createNode(
            ...
            node = composeTestRule.onNode(
                    matcher = matchers.getMatcher(),
                    useUnmergedTree = useUnmergedTree,
            )
        }

        override fun waitForDisplay() {
    ...
    data class Group(
        ...
        override fun plus(other: CodexNodeGroupAction): CodexNodeInfo {
            return copy(actions = actions + other)
        }
        ...
        override fun createNode(
            ...
            node = composeTestRule.onAllNodes(
                    matcher = matchers.getMatcher(),
                    useUnmergedTree = useUnmergedTree,
            )
        }
    ...
    /**
     * Waits for the display of the node or first in the group of nodes
     */
    open fun waitForDisplay(): Unit = throw NotImplementedError()
