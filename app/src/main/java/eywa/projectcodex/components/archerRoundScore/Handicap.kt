...
class Handicap {
    companion object {
        ...
        fun getHandicapForRound(
            ...
            while (true) {
                ...
                val testHCScore =
                        getScoreForRound(round, roundArrowCounts, roundDistances, testHC, innerTenArcher, arrows)
                when {
                    testHCScore == score -> {
                        // Some scores can have multiple possible handicaps - find the worst (highest)
                        while (
                            getScoreForRound(
                                    round, roundArrowCounts, roundDistances, testHC + 1, innerTenArcher, arrows
                            ) == score
                        ) {
                            testHC++
                        }
    ...
    enum class ScoringType(
        ...
        fun averageScorePerArrow(
            ...
            val sigma = (rangeInM * 1.036.pow(handicap + 12.9)
                    * 0.05
